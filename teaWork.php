...
if (empty($_POST["selsub"])) { // 不搜索默认显示所有记录
    $sql = "select uid,username,class,workName,workAddress from work where courseName = '$course'";
    $res = $mysqli->query($sql);

} else {
    $sel = $_POST["sel"]; // 模糊搜索，只要有关键词匹配即选中，关键词为用户名、姓名和班级
    $sql = "select uid,username,class,workName ,workAddress from work where (uid like '%$sel%' or username like '%$sel%' or class like '%$sel%') and courseName = '$course';";
    $res = $mysqli->query($sql);
...
while ($row = mysqli_fetch_array($res)) {
    ...
    echo "<td>$row[0]</td><td>$row[1]</td><td>$row[2]</td><td>$row[3]</td><td><a href=$row[4] download=$row[3]>下载</a></td>";
    echo '</tr>';
}
?>
        </table>
